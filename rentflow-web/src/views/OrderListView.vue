...
function needsUrgentConfirmation(order: Order) {
  if (order.effectiveStatus !== 'PENDING_CONFIRMATION') return false
  const millisecondsLeft = new Date(order.expiresAt).getTime() - now.value
  return millisecondsLeft > 0 && millisecondsLeft <= 5 * 60 * 1000
}
    ...
    <div v-else-if="result.items.length" class="order-list">
      <article v-for="order in result.items" :key="order.orderId" class="order-row" :class="{ 'order-row--urgent': needsUrgentConfirmation(order) }" @click="openOrder(order)">
        ...
        <div class="order-amount">
          ...
          <span v-if="needsUrgentConfirmation(order)" class="urgent-confirmation">即将过期，请尽快确认</span>
        </div>
      </article>
    </div>
