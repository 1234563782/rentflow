...
| 核心目标 | 完成“查询商品 → 选择租期 → 获取报价 → 预占商品时段容量并冻结价格 → 创建订单 → 出库前分配具体设备”的预订闭环 |

## 2. 产品背景

用户在短期拍摄、旅行、直播、会议或临时办公等场景下，需要租赁相机、镜头、无人机、电脑等数码设备。租赁系统如果只维护没有时间维度的商品总量，容易在租期重叠、预占过期和多人并发下发生超卖。

RentFlow 第一版对同型号高度同质的设备采用两阶段分配：预订时按“商品 + UTC 小时时间槽”原子预占容量，不分配具体设备；订单确认后、出库前再分配设备实例。容量槽使用数据库事务、稳定锁顺序和锁后复检，保证最后一个容量名额不会被多个请求同时占用。

V1 定位为**租赁预订闭环**，终点是创建待确认订单并确认至 `CONFIRMED`，不包含支付、交付、实际租赁开始、归还或售后处理，因此不称为完整租赁履约闭环。
...
1. 用户可以登录平台并保持认证状态。
2. 用户可以按关键词和租赁时间搜索商品。
3. 系统可以根据商品在指定时间槽内的剩余容量判断是否可租。
4. 用户可以创建有明确有效期的库存预占。
5. 系统可以根据租期计算租金与押金，并生成不可变的价格快照。
6. 用户可以创建带 15 分钟库存预占的待确认订单，并确认或取消订单。
7. 用户可以查看自己的订单列表和订单详情。
8. GearMate 可以调用 RentFlow API，基于真实商品、库存和报价回答用户问题。
9. 系统可以正确处理最后一个容量名额的并发争抢、预占过期和重复下单。

### 3.2 成功标准

- 两个用户同时争抢同一租期的最后一个容量名额时，只允许一个用户预占成功。
- 有效预占和已确认订单占用的时间槽容量不能被重复扣减。
- 预占到期后，对应时间槽容量可以立即再次被其他用户预占，不依赖清理任务。
...
- 一个商品可以对应多台设备实例。
- **容量槽 Capacity Slot**：某个商品在一个 UTC 小时时间槽内可被预订的容量。
- 预订阶段以商品时间槽容量为库存事实，不分配设备实例；已确认订单在出库前才分配具体设备编号。
- 起始时间向下取整到整点，结束时间向上取整到整点，覆盖区间仍按 `[start_at, end_at)` 处理。按小时取整会保守占用首尾不足一小时的容量。
- 设备实例总数或可租状态变化时，必须与容量调整使用同一套商品级串行化规则，禁止在已有预订低于新容量前直接缩容。
...
预占用于在用户确认订单前暂时锁定商品在租期覆盖时间槽内的一个容量名额，并冻结创建预占时的报价快照；此时 `equipment_unit_id` 为空。
...
- 预占成功后返回预占记录、到期时间、`source_quote_id` 和冻结价格快照，不返回具体设备编号。
- 预占到期或订单取消后不再占用容量；订单确认后预占转为 `CONSUMED`，对应容量 claim 持续有效直至订单不再占用该租期。
- 创建待确认订单必须携带 `Idempotency-Key`，库存预占不提供独立写入口；相同用户、相同接口、相同幂等键和相同请求返回原订单。
...
1. 创建待确认订单必须提交 `quote_id` 和 `Idempotency-Key`；同一事务创建订单与库存预占。
2. 一个 `quote_id` 最多成功创建一个预占。创建预占时必须锁定报价记录。
3. 创建预占时，在同一事务内校验报价属于当前用户且未过期，并校验商品和租期与报价完全一致。
4. 校验通过后生成租期覆盖的 UTC 小时时间槽，按时间升序锁定“商品 + 时间槽”记录，锁后统计有效 claim；容量充足时写入预占及每个时间槽的 claim，并记录 `source_quote_id`。
...
11. 确认订单只消费预占及其冻结价格快照，不重新计算价格。
12. 确认订单时使用 `SELECT ... FOR UPDATE` 锁定订单和预占记录，并再次校验归属、`effectiveStatus` 和 `start_at > CURRENT_TIMESTAMP(6)`。
...
第一版订单覆盖待确认、确认、取消、超时和基础查询，不处理支付、交付和归还。
...
- 用户已认证。
- 报价属于当前用户且有效、未被使用。
- 创建订单与预占商品时间槽容量在同一事务完成。
- 租期尚未开始，即 `start_at > CURRENT_TIMESTAMP(6)`。
- 预占中的冻结价格快照完整有效。
- 商品当前可租设备容量大于对应时间槽的有效 claim 峰值。
...
用户选择开始时间和结束时间
  → 输入关键词或浏览商品
  → Java 查询匹配商品
  → Java 根据商品容量和各小时时间槽的有效 claim 计算可租状态
  → 返回商品列表、可租数量和最低参考价格
```
...
```text
用户点击“立即预订”
  → 前端提交 quote_id 和 Idempotency-Key 创建待确认订单
  → Java 锁定报价记录
  → Java 校验报价归属、有效期、未被消费、商品和租期
  → Java 校验用户有效预占数量上限
  → Java 生成租期覆盖的 UTC 小时时间槽
  → 按时间升序锁定商品容量槽
  → 锁后重新统计 ACTIVE 未过期或 CONSUMED 的有效 claim
  → 容量充足时写入预占和时间槽 claim，不分配具体设备
  → 同一事务内写入预占、待确认订单、价格快照和 Outbox 事件
  → 返回待确认订单和到期时间
```

并发要求：

- 同一商品只剩最后一个容量名额时，并发请求只能有一个成功。
- 锁定顺序应稳定，降低死锁概率。
- 发生数据库死锁时只能有限重试。
- 锁后发现任一时间槽容量不足时返回库存不足，不得超卖。

### 7.5 下单流程

```text
用户进入订单确认页
  → 前端显示订单倒计时、“具体设备将在出库前分配”和价格快照
  → 用户确认或取消订单
  → 前端生成或复用 Idempotency-Key
  → Java 锁定订单和预占，校验用户、有效期和冻结价格快照
  → 确认时订单变为 CONFIRMED 并消费预占；取消时订单变为 CANCELLED 并释放预占
  → 返回订单详情
```
...
- 订单基本信息
- 商品快照
- 设备分配状态；尚未分配时显示“待分配设备”，分配后显示设备实例展示编号
- 租赁时间
- 价格明细和规则快照
- 状态历史

设备序列号等内部敏感字段不应完整暴露给普通用户。运营管理员在订单确认后、租期开始前执行出库分配；分配事务锁定订单和候选设备，锁后复检半开区间冲突并同时更新订单与来源预占。
...
| 编号 | 需求 |
|---|---|
| INV-01 | 根据商品和租期查询所有覆盖小时时间槽中的最小可租容量。 |
| INV-02 | 租期使用半开区间；容量槽按 UTC 小时生成，开始向下取整、结束向上取整。 |
| INV-03 | `ACTIVE` 且未逻辑过期的预占以及 `CONSUMED` 预占均计入容量占用。 |
| INV-04 | 已过期预占不得阻塞新请求。 |
| INV-05 | 查询结果只作即时参考，最终结果以预占事务为准。 |
| INV-06 | 非可租状态的设备实例不计入商品容量，也不得在出库分配时作为候选。 |
...
| RES-02 | 预占成功时只分配商品时间槽容量，`equipmentUnitId` 和 `equipmentDisplayCode` 保持为空。 |
| RES-03 | 预占具有可配置的到期时间。 |
| RES-04 | 预占创建必须按升序锁定全部容量槽，并在持锁后重查每个槽的有效 claim。 |
| RES-05 | 用户取消待确认订单时，系统在同一事务释放未消费的有效预占。 |
...
| 编号 | 需求 |
|---|---|
| ORD-01 | 用户可以基于有效报价创建带商品时间槽容量预占的待确认订单。 |
| ORD-02 | 创建、确认和取消订单必须提供 `Idempotency-Key`。 |
| ORD-03 | 订单保存商品、价格规则和金额快照。 |
| ORD-04 | 订单保存 `PENDING_CONFIRMATION`、`CONFIRMED`、`CANCELLED`、`EXPIRED` 状态和完整历史。 |
| ORD-05 | 用户只能查询自己的订单。 |
| ORD-06 | 重复请求不得重复创建订单。 |
| ORD-07 | 订单或预占过期、租期已经开始或状态变化时返回冲突，不得确认。 |
| ORD-08 | 创建待确认订单必须原子创建预占；确认订单必须锁定两者并通过条件更新消费预占。 |
| ORD-09 | 订单创建、确认、取消和超时事务必须写入 Transactional Outbox 事件。 |
| ORD-10 | 只有管理员可以为 `CONFIRMED` 且租期未开始的订单分配具体设备；重复分配同一订单应幂等返回已有结果。 |
| ORD-11 | 出库分配必须锁定订单和候选设备，并使用半开区间复检设备与其他已确认订单的租期冲突。 |
...
订单列表支持待确认、已确认、已取消和已过期状态；待确认订单显示倒计时和继续确认入口。
...
允许转换为 `PENDING_CONFIRMATION → CONFIRMED|CANCELLED|EXPIRED`。查询额外返回按数据库时间计算的 `effectiveStatus`，清理任务负责将逻辑过期状态收敛为持久化 `EXPIRED`。
...
| `ORDER_EXPIRED` | 409 | 待确认订单已经超过确认期限 |
| `ORDER_STATE_CONFLICT` | 409 | 当前订单状态不允许执行确认或取消 |
...
- 待确认订单、库存预占、价格快照、状态历史和 Outbox 事件使用同一个 MySQL 事务。
- 关键库存 SQL 必须显式实现，不使用内存锁代替数据库一致性。
- 确认订单与消费预占、取消订单与释放预占必须分别在同一事务完成。
- RabbitMQ 不参与库存正确性判断；Outbox 发布采用至少一次投递，消费者按稳定 `eventId` 幂等去重。
- V1 默认启用 RabbitMQ；MQ 不可用时核心订单事务仍可提交，未发布事件保留在 Outbox 并有限退避重试。
- 金额不得使用浮点数。
...
- 报价有效时间
- 幂等请求等待时间和记录保留期
- RabbitMQ 地址、凭据、发布确认超时、Outbox 批量大小和轮询间隔
- Agent 运行事件保留期
- Agent 默认模型提供方和模型 ID
- 模型 API 地址和凭据
- RentFlow 内部 API 地址
...
2. 最后一个时间槽容量名额的并发争抢测试稳定通过。
3. 预占逻辑过期后容量无需等待清理任务即可重新分配。
4. 重复下单不会创建重复订单。
